...
## Estado y pendientes
Ver **`PENDIENTES.md`** (nota viva, actualízala al hacer cambios). Estado actual: GitHub, Vercel y dominio `armandoruizmc.com` ya están conectados en cuentas del diputado; Supabase Auth ya recibe registro, confirmación por correo y login en producción; Resend verificó el dominio, Custom SMTP quedó configurado, Vercel ya tiene `RESEND_API_KEY` y `APPOINTMENTS_ADMIN_EMAIL`, y la migración `20260618_appointment_email_notifications.sql` está aplicada. Prioridad inmediata: probar correos de citas en producción, revisar plantillas Auth de Supabase, asegurar Redirect URLs de Supabase Auth, completar Aviso de Privacidad + revisión jurídica, contenido real (foto del diputado, contacto del footer, token de Instagram, casos reales de Donar), recordatorios por WhatsApp y "Mi Expediente" (subir documentos).
