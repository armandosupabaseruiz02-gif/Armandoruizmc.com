...
- [x] **React Doctor seguridad (2026-07-12):** agregado `doctor.config.json` para excluir del escaneo el build local generado `.next.nosync/**` (causaba falsos positivos en chunks/sourcemaps de dependencias por el `distDir` local anti-iCloud). Agregada migracion `20260712073527_use_private_is_admin_for_initiatives.sql` para que `initiatives_admin_all` use `private.is_admin()` en vez del helper publico revocado.
...
- [x] **Huevos de pascua del bot (2026-07-12):** el asistente responde mensajes secretos del equipo ante dos frases exactas ("meestoycagando.1" y "osvajotoputo"), definidas en `easterEggs` dentro de `src/lib/orientation/knowledge.ts`. Se evalúan antes del mapa de palabras clave, no aparecen en sugerencias y no afectan la orientación normal.
