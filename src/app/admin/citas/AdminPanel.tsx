...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  completed:            { label: "Completada",         color: "text-emerald-700 bg-emerald-100 border-emerald-200" },
...
export default function AdminPanel({
  ...
  async function handleSaveLink(id: string, link: string) {
    ...
    if (link.trim() && !normalizedLink) {
      setError("El enlace debe empezar con http:// o https:// y ser una URL válida.");
      return;
    }
    ...
    const { error: updateError } = await supabase.from("appointments")
      .update({ meeting_link: normalizedLink || null })
      .eq("id", id);
    if (updateError) setError("No se pudo guardar el enlace de videollamada.");
    else {
      if (normalizedLink) {
        await notifyAppointmentStatus(id, "meeting_link_updated");
      }
      router.refresh();
    }
    setLoading(null);
    ...
    <div className={`bg-white rounded-2xl border-2 transition-all duration-200
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className={`hidden sm:inline-flex items-center gap-1 px-3 py-1 rounded-full
                            ...
                            ${isOnline
                              ? "text-naranja-700 bg-naranja-100 border-naranja-200"
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-gray-100 pt-4">
          ...
          {isOnline && (
            <div className="mb-4 p-4 rounded-xl bg-naranja-50 border border-naranja-200">
              <p className="text-[11px] font-black text-naranja-700 uppercase tracking-wide mb-2 flex items-center gap-1.5">
              ...
              {showActions && a.status === "confirmed" ? (
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border-2 border-naranja-200 focus:border-naranja-400
                ...
                <a href={a.meeting_link} target="_blank" rel="noopener noreferrer"
                   className="text-[14px] text-naranja-700 font-semibold underline break-all">
                  {a.meeting_link}
                </a>
              ) : (
